...
class _MarkdownExtra_TmpImpl extends \Michelf\Markdown {
	...
	protected function doExpandoBlocks($text) {
	#
	# Adding the expando block syntax to regular Markdown:
	#
	# ++++++
	# Code block
	# ++++++
	#
		...
		$text = preg_replace_callback('{
			...
			array(&$this, '_doExpandoBlocks_callback'), $text);

		return $text;
	}
	protected function _doExpandoBlocks_callback($matches) {
		$classname =& $matches[2];
		$attrs     =& $matches[3];
		$content   =  $matches[4];

        $html = $this->runBlockGamut($content);

		if ($classname != "") {
			if ($classname{0} == '.') $classname = substr($classname, 1);
			$attr_str = ' class="extra-info '.$this->code_class_prefix.$classname.'"';
		} else {
			$attr_str = ' class="extra-info" ' . $this->doExtraAttributes($this->code_attr_on_pre ? "pre" : "code", $attrs);
		}
		$block  = '<div' . $attr_str . '><div class="content">' . $html . '</div></div>';
